...
#
#   wiki_backfill list [--since DATE] [--min-prompts N] [--project GLOB]
...
#   wiki_backfill reduce TRANSCRIPT [OUT]
...
#                                      stdout.
#   wiki_backfill brief                Print the path of the digest brief to
#                                      hand a sub-agent along with a reduced
#                                      transcript and an output path.
#
# The jq filter and the brief live next to this script in wiki_backfill/.

set -uo pipefail

projects_dir="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/projects"
assets_dir="$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/wiki_backfill"
...
cmd_reduce() {
  ...
  [ -n "$transcript" ] && [ -f "$transcript" ] || die "usage: wiki_backfill reduce TRANSCRIPT [OUT]"
  if [ -n "$out" ]; then
...
[ $# -ge 1 ] || die "usage: wiki_backfill list|reduce|brief ..."
